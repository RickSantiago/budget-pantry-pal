    ...
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            {sortedLists.map(list => {
                ...
                const uniqueCategories = [...new Set(list.items?.map(item => item.category || "Outros"))];
                const MAX_ICONS = 6;
                const displayedCategories = uniqueCategories.slice(0, MAX_ICONS);

                return (
                    <Card 
                        key={list.id} 
                        onClick={() => onSelectList(list.id)} 
                        className={`cursor-pointer transition-all duration-300 hover:border-primary/60 hover:shadow-lg glass flex flex-col justify-between ${currentListId === list.id ? 'border-primary/80 shadow-xl' : 'border-border/50'}`}>
                        <div>
                               ...
                               {budget > 0 && (
                                ...
                                <div className="flex items-center gap-3 pt-2 border-t border-border/50">
                                    {displayedCategories.map(category => {
                                        ...
                                        return <Icon key={category} className={`h-5 w-5 ${color}`} />
                                    })}
                                    {uniqueCategories.length > MAX_ICONS && (
                                        <span className="text-xs text-muted-foreground">+{uniqueCategories.length - MAX_ICONS}</span>
                                    )}
                                </div>
                            </CardContent>
                        </div>
